document.addEventListener("DOMContentLoaded", () => {
    ...
    // --- NEW LOGIC: Separate event listeners for the two buttons ---
    
    // Event listener for the "Plan Your Trip" button
    document.getElementById('plan-trip-btn').addEventListener('click', () => {
        ...
        if (destination.toLowerCase() === 'manali') {
            window.location.href = "pages/manali.html";
    ...
    // Event listener for the "Get Predictions" button
    document.getElementById('prediction-btn').addEventListener('click', getPredictions);

    // --- END OF NEW LOGIC ---

    // The main function for getting predictions (no redirect logic inside)
    async function getPredictions() {
        const destination = document.getElementById('destination-input').value.trim();

        // Validation (this is a shared function now, so keep validation here)
        if (!destination) {
